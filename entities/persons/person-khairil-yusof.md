---
id: person-khairil-yusof
type: person
name: Khairil Yusof
status: active
confidence: high
tags: [malaysia, southeast-asia, kuala-lumpur, civic-tech, digital-rights, open-data, anti-corruption, transparency, internet-freedom, open-government, co-founder, programmer]
created: 2026-06-10
last_updated: 2026-06-10
affiliations:
  - org: org-sinar-project
    role: co-founder and coordinator
    period: 2010-
public_profiles:
  - platform: personal-site
    url: https://kaeru.my/
  - platform: twitter
    url: https://x.com/kaerumy
  - platform: linkedin
    url: https://www.linkedin.com/in/khairil/
sources:
  - url: https://sinarproject.org/about/team
    last_checked: 2026-06-10
    note: 'Sinar Project team page — primary source for Khairil Yusof''s current title (Coordinator) and role as co-founder; technical background in software development with open source software (FreeBSD, Linux, Python); focus on innovative open-data and anti-corruption methods in constrained environments'
  - url: https://splicemedia.com/stories/malaysias-sinar-project-government-data
    last_checked: 2026-06-10
    note: 'Splice Media profile — primary source for Yusof''s founding role: part of a cohort of Malaysian "tech types" galvanized by the Bersih 2.0 rally in 2011, emerging from Malaysia''s civic activism and open-data scene; Sinar Project operated from his Kuala Lumpur studio apartment on grants and donations; anti-corruption contract-database work led to corruption charges against a politician'
  - url: https://medium.com/berkman-klein-center/open-without-freedom-8faf1df6e45b
    last_checked: 2026-06-10
    note: 'Berkman Klein Center / Internet Monitor interview with Yusof — primary source for Sinar Project''s internet censorship monitoring work beginning in 2013, and for his advocacy of strategic litigation to challenge internet blocking as potentially unconstitutional; Twitter handle @kaerumy confirmed'
---
# Khairil Yusof


Co-founder and coordinator of [Sinar Project](../organizations/org-sinar-project.md), the Kuala Lumpur-based civic-technology organization whose work spans parliamentary transparency, internet-freedom monitoring, and AI governance. [Emerging from the Malaysian open-data activist scene galvanized by the Bersih 2.0 rally in July 2011](https://splicemedia.com/stories/malaysias-sinar-project-government-data), Yusof leads Sinar Project's programme design and technical implementation — a background in open-source software development (FreeBSD, Linux, Python) underpinning tools from parliamentary written-reply archives and anti-corruption contract databases to OONI-based censorship dashboards — and [advocates for strategic litigation as a mechanism to challenge internet blocking as potentially unconstitutional](https://medium.com/berkman-klein-center/open-without-freedom-8faf1df6e45b).

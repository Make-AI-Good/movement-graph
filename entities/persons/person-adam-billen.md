---
id: person-adam-billen
type: person
name: Adam Billen
status: active
confidence: medium
tags: [staff, public-policy]
created: 2026-05-08
last_updated: 2026-05-08
affiliations:
  - org: org-encode-justice
    role: co-executive director, public policy
    period: 2024-
public_profiles:
  - platform: encode-ai
    url: https://encodeai.org/
sources:
  - url: https://encodeai.org/
    last_checked: 2026-05-08
    note: Encode AI's current operating site, where staff leadership including Billen as co-executive director is publicly listed
  - url: https://stanforddaily.com/2024/10/20/gov-newsom-vetoes-ai-safety-bill/
    last_checked: 2026-05-08
    note: Stanford Daily coverage of California SB 1047 in which Encode senior staff including Billen feature in the policy advocacy record
---

Co-executive director for public policy at [Encode Justice](../organizations/org-encode-justice.md) (operating publicly as Encode / Encode AI), where he leads federal policy advocacy.

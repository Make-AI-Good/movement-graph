...
Pass 2 context: pass 1 (entity_hash a2b5ca37, 2026-05-22) flagged two discrepancies (claims 4 and 21) and one unverifiable (claim 14). The Editor's audit-D backfill applied both single-token fixes: "Research / Information and Society" → "Information & Society" (claim 4) and "January 2024" → "November 2023" (claim 21, applied in both body and `sources[8].note` frontmatter prose). This pass re-fetches the two affected canonical sources to re-verify the corrected claims. The remaining 18 claims are bit-identical to pass 1's body text; their pass 1 source-quote attestations are referenced verbatim and the verified status carries forward — each is explicitly noted as such so a future re-auditor can re-check the chain.
